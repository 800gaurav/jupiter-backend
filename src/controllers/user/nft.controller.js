...
if(user.nftbuyOTP !== Number(otp)){
    ...
    user.fundBalance -= nft.price;
    user.totalInvested += nft.price;
    referrar.walletBalance += nft.price * 0.03
    referrar.todayIncome += nft.price * 0.03
    referrar.proBonusIncome += nft.price * 0.03;
    referrar.proBonusHistory = referrar.proBonusHistory || [];
referrar.proBonusHistory.push({
  fromUser: user.userId,
  amount: nft.price * 0.03,
  baseAmount: nft.price,
  date: new Date(),
});
    user.nfts.push({ nft: nft._id, price: nft.price });
